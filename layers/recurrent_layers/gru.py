...
class GRU(AdaptiveObject):
    ...
    def __call__(self, inputs: xp.ndarray) -> xp.ndarray:
        """
        The forward pass assumes that new_input is a three-dimensional array of shape Nb × T × D,
        where Nb is the batch size, T is the sequence length, and D is the input dimension.

        The forward pass is implemented so that the signal is propagated step by step:
        we iterate along the second dimension of new_input and compute the outputs and hidden state one timestep at a time.
        """
    ...
    def backward(self, dEdO: xp.ndarray) -> xp.ndarray:
        ...
        for k in reversed(range(1, T+1)):
            ...
            dEdU[:, k-1, :] = xp.matmul(dEdAz, self._Vz) + xp.matmul(dEdAh, self._Vh) + xp.matmul(dEdAr, self._Vr)
