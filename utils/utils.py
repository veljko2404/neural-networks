...
def generate_vocabulary(text: str, padding: str = None) -> (dict, dict):
...
def generate_sequences(text: str, seq_delimiter: Optional[str] = '\n',
                       seq_length: Union[int, str] = 'auto', padding: str = ".") -> (np.ndarray, np.ndarray):
...
def get_8x8_mnist_data(flat_images: bool = False, max_num_of_samples: int = None, max_num_of_classes: int = None) -> (np.ndarray, np.ndarray):
    ...
    data_numpy = np.hstack((t.reshape(-1, 1), X_flat))
    ...
    y = np.zeros((len(X), num_of_classes))
...
def to_one_hot(t: np.ndarray, num_of_classes: int):
    tmp = np.zeros((t.size, num_of_classes), dtype=float)
    tmp[np.arange(0, t.size), t.astype(int).squeeze()] = np.ones(t.size)
    return tmp


def get_mnist_data(flat_images: bool = False, max_num_of_samples: int = None, max_num_of_classes: int = None, _8x8: bool = False) -> (np.ndarray, np.ndarray):
    ...
    if _8x8:
        ...
        data_numpy = np.array(mnist_dataframe.to_numpy())
        im_size = 28
